...
const float knee_side_sign[4] = {-0.6429f, 0.6429f, -0.6429f, 0.6429f};
...
void spi_send_receive(spi_command_t *command, spi_data_t *data) {
  ...
  /*static int num_count=0;
    ...
    num_count  =1;
       ...
       command->q_des_abad[0]=num_count;
       command->q_des_abad[1]=num_count;
       command->q_des_abad[2]=num_count;
       command->q_des_abad[3]=num_count;
      command->q_des_hip[0]=num_count;
       command->q_des_hip[1]=num_count;
       command->q_des_hip[2]=num_count;
       command->q_des_hip[3]=num_count;
       command->q_des_knee [0]=num_count;
       command->q_des_knee[1]=num_count;
       command->q_des_knee[2]=num_count;
       command->q_des_knee[3]=num_count;

        command->qd_des_abad[0]=num_count;
       command->qd_des_abad[1]=num_count;
       command->qd_des_abad[2]=num_count;
       command->qd_des_abad[3]=num_count;
      command->qd_des_hip[0]=num_count;
       command->qd_des_hip[1]=num_count;
       command->qd_des_hip[2]=num_count;
       command->qd_des_hip[3]=num_count;
       command->qd_des_knee [0]=num_count;
       command->qd_des_knee[1]=num_count;
       command->qd_des_knee[2]=num_count;
       command->qd_des_knee[3]=num_count;

      command->kp_abad[0]=num_count;
       command->kp_abad[1]=num_count;
       command->kp_abad[2]=num_count;
       command->kp_abad[3]=num_count;
      command->kp_hip[0]=num_count;
       command->kp_hip[1]=num_count;
       command->kp_hip[2]=num_count;
       command->kp_hip[3]=num_count;
       command->kp_knee [0]=num_count;
       command->kp_knee[1]=num_count;
       command->kp_knee[2]=num_count;
       command->kp_knee[3]=num_count;

       command->kd_abad[0]=num_count;
       command->kd_abad[1]=num_count;
       command->kd_abad[2]=num_count;
       command->kd_abad[3]=num_count;
      command->kd_hip[0]=num_count;
       command->kd_hip[1]=num_count;
       command->kd_hip[2]=num_count;
       command->kd_hip[3]=num_count;
       command->kd_knee [0]=num_count;
       command->kd_knee[1]=num_count;
       command->kd_knee[2]=num_count;
       command->kd_knee[3]=num_count;

       command->tau_abad_ff[0]=num_count;
       command->tau_abad_ff[1]=num_count;
       command->tau_abad_ff[2]=num_count;
       command->tau_abad_ff[3]=num_count;
      command->tau_hip_ff[0]=num_count;
       command->tau_hip_ff[1]=num_count;
       command->tau_hip_ff[2]=num_count;
       command->tau_hip_ff[3]=num_count;
       command->tau_knee_ff [0]=num_count;
       command->tau_knee_ff[1]=num_count;
       command->tau_knee_ff[2]=num_count;
       command->tau_knee_ff[3]=num_count;
...
//  using std::ofstream;
  //ofstream out;
  // out.open("spi_data.txt",ofstream::app);
/* 
static float lastn=command->q_des_abad[0];
if(lastn==command->q_des_abad[0])
{
  printf("same\r\n");
} 

lastn=command->q_des_abad[0];
*/
 ...
 //out<<"spi2spine:"<<g_spine_cmd.tau_abad_ff[spi_board]<<"  "<<g_spine_cmd.tau_hip_ff[spi_board]<<"  "<<g_spine_cmd.tau_knee_ff[spi_board]<<"  ";
    ...
    if(spi_board==1)
    {
      printf("%f\r\n",g_spine_data.q_abad[0]);
      static float last_data=g_spine_data.q_abad[0];
      if(g_spine_data.q_abad[0]-last_data>=2)
      {
        error_count++;
     }
    last_data=g_spine_data.q_abad[0];
    printf("%d\r\n",error_count);
  ...
  // float qd_knee[2];
  // int32_t flags[2];
    //out<<"spine2spi:"<<g_spine_data.q_abad[spi_board]<<"  "<<g_spine_data.q_hip[spi_board]<<"  "<<g_spine_data.q_knee[spi_board]<<std::endl;
  }
