...
void RobotRunner::init() {
  ...
  // Always initia_legsEnabledlize the leg controller and state entimator
...
{
  if(spiData->q_abad[leg]>2.3562||spiData->q_abad[leg]<-2.3562)
  flag=1;
}

    if( ((rc_control.mode == 0) && controlParameters->use_rc )||flag==1) {
	    _legController->setEnabled(false);
