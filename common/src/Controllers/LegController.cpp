...
void LegController<T>::updateData(const SpiData* spiData) {
  for (int leg = 0; leg < 4; leg++) {
    ...
    computeLegJacobianAndPosition<T>(_quadruped, datas[leg].q, &(datas[leg].J),
                                     &(datas[leg].p), leg);
                                   // printf("leg(%d)->q_abad=%f ",leg,spiData->q_abad[leg]);
                                  //   printf("leg(%d)->q_hip=%f ",leg,spiData->q_hip[leg]);
                                  // printf("leg(%d)->q_knee=%f\r\n",leg,spiData->q_knee[leg]);
                                     printf("leg(%d)->x=%f\t",leg,datas[leg].p.operator()(0));
                                   printf("leg(%d)->y=%f\t",leg,datas[leg].p.operator()(1));
                                     printf("leg(%d)->z=%f\r\n",leg,datas[leg].p.operator()(2));
    // v
